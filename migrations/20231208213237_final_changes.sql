...
CREATE TABLE knowledge_competency ( -- Связь между компетенцией и составляющим ее знанием
    knowledge_id UUID REFERENCES knowledge(knowledge_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE competency_profession ( -- Связь между профессией и требующейся для нее компетенции
    competency_id UUID REFERENCES competencies(competency_id) ON DELETE CASCADE ON UPDATE CASCADE,
    profession_id UUID REFERENCES professions(profession_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE project_portfolio ( -- Связь между проектом и портофлио проектов конкретного студента.
    project_id UUID REFERENCES projects(project_id) ON DELETE CASCADE ON UPDATE CASCADE,
    portfolio_id UUID REFERENCES portfolios(portfolio_id) ON DELETE CASCADE ON UPDATE CASCADE, 
    team_role VARCHAR,
